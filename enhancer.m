function [ I, R] = enhancer( src, alpha, beta, powerI, powerR, vareps, r, r0, K, debug)
    ...
    for iter = 1:K
        ...
        ux = max(abs(avgIx.^powerI),eps).^(-1);  % ux in Eq.(11) avgIx.^2 > avgIx.*Ix > Ix.^2
        uy = max(abs(avgIy.^powerI),eps).^(-1);  % uy in Eq.(11) similarly
        ux(:,end) = 0;
        uy(end,:) = 0;
        ...
        vx = max(abs(avgRx.^powerR),eps).^(-1);                            % vx in Eq.(11)
        vy = max(abs(avgRy.^powerR),eps).^(-1);                            % vy in Eq.(11)
        vx(:,end) = 0;
        vy(end,:) = 0;
